...
public class ProductControllerIntegrationTest {
    ...
    @Test
    @WithMockUser // No roles specified, simulating a user with no roles
    public void testAccessRestrictedEndpointWithNoRoles() throws Exception {
        // Execute the HTTP GET request and expect a 403 Forbidden status
        mockMvc.perform(MockMvcRequestBuilders
                .get("/products?activeFilter=true&searchterm=D")
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isForbidden()); // Expecting a 403 Forbidden
    }
